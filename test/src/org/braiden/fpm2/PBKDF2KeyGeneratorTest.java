...
public class PBKDF2KeyGeneratorTest extends TestCase {
	...
	public void testHmacSha256() throws Exception
	{
		...
		PBKDF2KeyGenerator keyGenerator = new PBKDF2KeyGenerator(salt.getBytes());
		byte[] key = keyGenerator.generateKey("secret");
		assertEquals(
	...
	public void testHmacSha1() throws Exception
	{
		...
		for (PBEKeySpec keySpec : tests) {
			...
			PBKDF2KeyGenerator myKeyGenerator = new PBKDF2KeyGenerator(keySpec.getSalt(), keySpec.getKeyLength() / 8, keySpec.getIterationCount(), "HmacSHA1");
			byte[] myKey = myKeyGenerator.generateKey(new String(keySpec.getPassword()));
